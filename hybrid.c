...
void *hybrid_thread(void *arg)
{
    ...
    while (1)
    {
        ...
        // Saving the current speed
        previous_speed = global_state.speed;

        if (global_state.battery_mode == 1)
        {
            global_state.battery_level -= 2.0f; // drain rate
            global_state.assist_active = 1;
            strcpy(global_state.hybrid_mode, "BATTERY");
            // fuel consumption near zero
            if (global_state.battery_level <= 0.0f)
            {
                global_state.battery_level = 0.0f;
                global_state.battery_mode = 0; // toggle off
                enqueue_event("Battery depleted - battery mode OFF");
            }
        }
